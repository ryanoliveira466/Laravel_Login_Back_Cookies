...
class PostController extends Controller
{
    ...
    public function publicIndexQuery(Request $request)
    {
        ...
        try {
            $posts = Post::with(['user:id,name,slug,photo,email'])
                ->select('user_id', 'name', 'description', 'javascript', 'css', 'html', 'slug', 'photo', 'tags')
    ...
    public function userProjects($slug)
    {
        try {
            ...
            $projects = Post::select('name', 'description', 'javascript', 'css', 'html', 'slug', 'photo', 'tags')->where('user_id', $user->id)->get();
            return response()->json([
                'success' => true,
